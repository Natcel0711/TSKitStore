<script lang="ts">
	...
	import { cart_capacity } from './cart';
	import '../app.css';
	export let data;
	...
	<div class="flex-1">
		{#if $page.data.user}
			...
			<form action="/logout" method="POST">
				<button type="submit" class="btn text-slate-300 btn-ghost normal-case text-xl"
					>Logout</button
				>
			</form>
		{/if}
				...
				<label tabindex="0" class="btn btn-ghost btn-circle">
					<div class="indicator">
						<svg
							...
							/></svg
						>
						<span class="badge badge-sm bg-primary indicator-item">{$cart_capacity}</span>
					</div>
				</label>
				<div tabindex="0" class="mt-3 card card-compact dropdown-content w-52 bg-base-100 shadow">
					<div class="card-body">
						<span class="font-bold text-lg">{$cart_capacity} Items</span>
